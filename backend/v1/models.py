...
class CustomUserManager(BaseUserManager):
    def create_user(self, email, name, password=None, role=UserRole.COMPANY, **extra_fields):
        if not email:
    ...
    def create_superuser(self, email, name, password=None, **extra_fields):
        ...
        return self.create_user(email, name, password=password, role=UserRole.ADMIN, **extra_fields)
...
class UserInvite(models.Model):
    id = models.CharField(primary_key=True, max_length=21, default=generate_nano_id, editable=False)
...
class EventsCompany(models.Model):
    event = models.ForeignKey(Event, on_delete=models.CASCADE, related_name='participating_companies')
    company = models.ForeignKey(Company, on_delete=models.CASCADE)
...
class EventsStaff(models.Model):
    id = models.CharField(primary_key=True, max_length=21, default=generate_nano_id, editable=False)
    event = models.ForeignKey(Event, on_delete=models.CASCADE, related_name='event_staffs')
...
class Check(models.Model):
    ...
    events_staff = models.ForeignKey(EventsStaff, on_delete=models.CASCADE, related_name='checks_history')
    user_control = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, related_name='checks_performed')
